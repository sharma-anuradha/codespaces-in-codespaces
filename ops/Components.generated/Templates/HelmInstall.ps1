...
function Main() {
  ...
  if (Select-Subscription) {
    Write-Host "Getting credentials for '{{{clusterName}}}'" -ForegroundColor DarkGray
    Import-AzAksCredential -ResourceGroupName '{{{baseRegionName}}}' -Name '{{{clusterName}}}' -Force -Verbose:$script:Verbose

    Write-Host "Installing chart '$chartName' to '{{{clusterName}}}' in namespace '{{{helmNamespace}}}'" -ForegroundColor green
    & helm.exe show all $chartFolder | Out-String | Write-Host -ForegroundColor DarkGray
    & helm.exe upgrade "${chartName}-release" "$chartFolder" --namespace "{{{helmNamespace}}}" --install | Out-String | Write-Host -ForegroundColor DarkGray
  }
}
