...
# with names file "codesp.prod-ctl-rel-us-w2.names.json"
...
function Main() {
  ...
  if (Select-Subscription) {
    Write-Host "Getting credentials for 'vscs-codesp-prod-rel-us-w2-cluster-v1'" -ForegroundColor DarkGray
    Import-AzAksCredential -ResourceGroupName 'vscs-codesp-prod-rel-us-w2' -Name 'vscs-codesp-prod-rel-us-w2-cluster-v1' -Force -Verbose:$script:Verbose

    Write-Host "Installing chart '$chartName' to 'vscs-codesp-prod-rel-us-w2-cluster-v1' in namespace 'default'" -ForegroundColor green
