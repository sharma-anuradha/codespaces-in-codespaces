...
function Main() {
  ...
  if (Select-Subscription) {
    Write-Host "Getting credentials for 'vscs-codesp-prod-rel-eu-w-cluster-v1'" -ForegroundColor DarkGray
    Import-AzAksCredential -ResourceGroupName 'vscs-codesp-prod-rel-eu-w' -Name 'vscs-codesp-prod-rel-eu-w-cluster-v1' -Force -Verbose:$script:Verbose

    Write-Host "Installing chart '$chartName' to 'vscs-codesp-prod-rel-eu-w-cluster-v1' in namespace 'default'" -ForegroundColor green
