...
[CmdletBinding()]
param (
  [string]$SubscriptionName = 'vscs-codesp-ppe-ctl',
  [string]$SubscriptionId = 'c57ef400-af62-4b47-a0b6-24ae55e7b8fd'
)
...
function Main() {
  ...
  if (Select-Subscription) {
    Write-Host "Getting credentials for 'vscs-codesp-ppe-rel-us-w2-cluster-v1'" -ForegroundColor DarkGray
    Import-AzAksCredential -ResourceGroupName 'vscs-codesp-ppe-rel-us-w2' -Name 'vscs-codesp-ppe-rel-us-w2-cluster-v1' -Force -Verbose:$script:Verbose

    Write-Host "Installing chart '$chartName' to 'vscs-codesp-ppe-rel-us-w2-cluster-v1' in namespace 'default'" -ForegroundColor green
    & helm.exe show all $chartFolder | Out-String | Write-Host -ForegroundColor DarkGray
    & helm.exe upgrade "${chartName}-release" "$chartFolder" --namespace "default" --install | Out-String | Write-Host -ForegroundColor DarkGray
  }
}
