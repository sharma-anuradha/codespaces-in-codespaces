...
if [ $help != 0 ] ; then
    ...
    echo "     --ssl-cert-secret-name             the azure key vault secret name for the ssl certificate (Kubernetes secret name: ssl-cert)"
    echo "     --ssl-cert-github-pf-secret-name   the azure key vault secret name for the ssl certificate used for github port forwarding (Kubernetes secret name: ssl-cert-github-pf)"
