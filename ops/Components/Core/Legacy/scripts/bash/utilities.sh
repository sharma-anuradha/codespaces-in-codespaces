...
function init_utilities()
{
    ...
    # Disable emitting color in azure devops releases because it makes the logs unreadable
    if isPipelineBuild; then
        utilities_color_info=""       # green
        utilities_color_debug=""      # dark gray
        utilities_color_error=""      # red
        utilities_color_verbose=""    # cyan
        utilities_color_warning=""    # yellow
        utilities_color_normal=""     # (unset)
    fi
}
...
function isPipelineBuild()
{
    # a pipeline is either a build or a release
    local build_id="$(set +u; echo "${BUILD_BUILDID}")"
    local release_definition="$(set +u; echo ${RELEASE_DEFINITIONNAME})"
    [ -z "${build_id}${release_definition}" ] && return 1 || return 0
}
...
function exec_dry_run_with_stdout()
{
...
# Splits a string on a delimiter and stores the result into an array.
# The input string may or may not include a trailing delimiter.  If it does, it is ignored.
#
# Usage:
#     split_string out_arr "1,2,3" ","
#     split_string out_arr "1,2,3," ","
#
#     Equivalent to: `declare -a out_arr=([0]="1" [1]="2" [2]="3")`
function split_string()
{
    local value="$2"
    local delim="$3"

    # Current version of `readarray` supports a -d option to specify the delimiter, but the version
    # in the pipeline doesn't seem to be up to date.  The workaround is to use `tr` to convert the 
    # delimiter to a newline which `readarray` expects to be the delimiter.
    readarray -t $1 < <(printf '%s' "${value}" | tr "${delim}" "\n")
}
