...
Write-Host "Onboarding V2 subscription '$($sub.Subscription.Name)'" -ForegroundColor Green

# Register ARM providers and features
...
if ($Plane -ne "data") {
    ...
    $opsSp = Get-ServicePrincipal -Prefix $Prefix -Component $Component -Env $Env -Plane "ops" -Create
...
$parameters = @{
    ...
    skipResources     = $DataType -or $Plane -eq 'data'
}
...
if ($opsSpId) {
    "Creating service principal certificate" | Out-String | Write-Host
    ...
    "Assigning service principal certificate credentials" | Out-String | Write-Host
