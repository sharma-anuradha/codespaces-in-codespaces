...
namespace Microsoft.VsSaaS.Services.CloudEnvironments.ResourceBroker
{
    ...
    public class ResourcePool : IResourcePool
    {
        ...
        public ResourcePool(IResourceRepository resourceRepository)
        ...
        public async Task<ResourceRecord> TryGetAsync(AllocateInput input, IDiagnosticsLogger logger)
        {
            ...
            while (item == null && trys < 3)
            {
                ...
                item = await ResourceRepository.GetUnassignedResourceAsync(input.SkuName, input.Type, input.Location, logger);
