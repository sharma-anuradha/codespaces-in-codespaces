...
namespace Microsoft.VsSaaS.Services.CloudEnvironments.BackEndWebApiClient
{
    ...
    {
        /// <summary>
        /// Add the back end <see cref="IHttpClientProvider"/> and <see cref="IResourceBrokerClient"/>.
        /// </summary>
        ...
        private static void AddResourceBrokerClient(this IServiceCollection services, bool useMocks)
        {
            if (useMocks)
            {
                services.AddSingleton<IResourceBrokerClient, MockResourceBrokerClient>();
            }
            else
            {
                services.AddSingleton<IResourceBrokerClient, HttpResourceBrokerClient>();
            }
        }
    }
}
