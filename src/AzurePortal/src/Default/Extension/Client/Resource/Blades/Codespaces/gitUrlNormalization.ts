import { SupportedGitService, getSupportedGitService } from './getSupportedGitService';
...
export function isRecognizedGitUrl(maybeUrl: string): boolean {
    ...
    try {
        ...
        if (
            service === SupportedGitService.Unknown ||
            (service === SupportedGitService.AzureDevOps && !evaluateFeatureFlag(azureDevOpsOAuth))
        ) {
            return false;
        }
    ...
    function isSupportedGithubPath(url: URL) {
        ...
        if (type === 'commit' || type === 'tree') {
            if (isGitHubRepositoryName(`${org}/${repository}`) && rest.length >= 1) {
                return true;
            }
        }
