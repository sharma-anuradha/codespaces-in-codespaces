...
import { PromiseCompletionSource, SshDisconnectReason } from '@vs/vs-ssh';
import { WorkspaceClient } from 'vso-ts-agent';
import { ILiveShareClient, vsls, Trace, createTrace } from 'vso-client-core';
...
export class LiveShareConnection implements Disposable {
    ...
    constructor(private readonly liveShareClient: ILiveShareClient, private sessionId: string) {
        this.logger = createTrace(`LiveShareConnection:${sessionId.substr(0, 5)}`);
    }
...
export class LiveShareConnectionFactory {
    ...
    constructor(private readonly liveShareClient: ILiveShareClient) {
        this.logger = createTrace('LiveShareConnectionFactory');
    }
