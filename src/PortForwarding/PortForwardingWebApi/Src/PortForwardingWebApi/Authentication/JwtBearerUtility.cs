...
using Microsoft.VsSaaS.Services.CloudEnvironments.PortForwardingWebApi.Extensions;
    ...
    public static class JwtBearerUtility
    {
        ...
        internal static async Task TokenValidatedAsync(TokenValidatedContext context)
        {
            ...
            try
            {
                ...
                {
                    var newPrincipal = await validatedPrincipalHandler.ValidatedPrincipalAsync(context.Principal, jwtSecurityToken, logger.NewChildLogger());
                ...
                {
                    var msg = "context.SecurityToken not JwtSecurityToken";
                    logger
                        .AddErrorDetail(msg)
                        .LogError("jwt_authentication_error");
