...
export class VSLSWebSocket implements IWebSocket {
    ...
    public send(data: ArrayBuffer | ArrayBufferView) {
      ...
      trace(`Ssh channel [${VSLSWebSocket.socketCnt}][${this.isChannelDisposed}] send: \n${bufferData.toString()}\n`);
      this.channel.send(bufferData);
    }
  
    public close() {
        trace(`Ssh channel[${VSLSWebSocket.socketCnt}] closed by the VSCode shell.`);
        this.channel.session.close(SshDisconnectReason.byApplication, 'Workbench requested to close the connection.');
    }
