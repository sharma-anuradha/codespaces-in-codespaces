...
async function downloadVSCodeAssetsInternal(quality, vscodeAssetsTargetPathBase, assetName) {
    ...
    console.log(`** requiredCommitId`, requiredCommitId);
...
async function linkBuiltinStaticExtensions(quality) {
    ...
    const commitId = getQualityCommitName(quality, requiredCommitId);
    const builtinExtensionsTargetPath = path.join(targetExtensionsFolderPath, commitId);
